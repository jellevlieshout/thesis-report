...
\begin{figure}[h!]
\centering
\fbox{\parbox[c][6cm][c]{0.9\textwidth}{\centering \textit{[Placeholder F-1: system architecture diagram]}}}
\caption{System architecture: FastAPI experiment runner, LangGraph workflows, Couchbase persistence (datasets, runs, predictions, evaluations), vLLM open-weights serving on the UPM A100 node, LangSmith tracing, and the React inspection frontend.}
...
\begin{figure}[h!]
\centering
\fbox{\parbox[c][6cm][c]{0.9\textwidth}{\centering \textit{[Placeholder F-2: monolithic vs.\ single-call structured output vs.\ 3-step pipeline]}}}
\caption{The three system architectures compared in this thesis. The monolithic baseline produces only a free-text rewrite; the single-call structured-output workflow commits to detected spans alongside the paraphrase; the 3-step agentic pipeline separates detection, explanation, and transformation into sequential calls with inspectable intermediate state.}
