...
\begin{itemize}
    ...
    \item \textbf{Open-weights serving}: vLLM (continuous batching, PagedAttention) on a UPM A100 GPU node, exposed to the experiment runner via a basic-auth-protected ngrok HTTPS tunnel. Two open-weights models are evaluated: \texttt{meta-llama/Meta-Llama-3.1-8B-Instruct} (FP16, deliverable system) and \texttt{casperhansen/llama-3.3-70b-instruct-awq} (AWQ-quantised, scale-comparator). The same OpenAI-compatible client code path serves both, so the only difference between conditions is the model identifier on the run document.
