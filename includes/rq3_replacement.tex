...
\begin{figure}[h!]
\centering
\fbox{\parbox[c][6cm][c]{0.9\textwidth}{\centering \textit{[Placeholder F-3: rating distributions per system and rubric dimension]}}}
\caption{Distribution of standardised ratings per system and rubric dimension. Simplicity is the weakest dimension for every system; the 70B comparator is rated lowest on simplicity.}
\label{fig:survey-distributions}
\end{figure}

% TODO(jelle): prose (four moves above).

\subsection{Qualitative Examples}
% --- GUIDE (~1 pp, ~400 words): correct / partially correct / incorrect
% replacements from the survey pool, ideally aligned with respondent
% free-text alternatives to show the system-vs-human gap. Good candidates
% from the engineering log: src_00 "Long story short" -> "To summarize"
% (clean hit); src_01 "bear the cross" (8B agentic missed the idiom,
% substituted within it); src_05 "poison pill" (pipeline detection miss ->
% input returned unchanged: rated high on grammar/meaning by construction,
% the honest failure mode of span-anchored systems).
